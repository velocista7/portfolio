...
/* Korean DatePicker */
(function(factory){if(typeof define==="function"&&define.amd){define(["../widgets/datepicker"],factory)}else{factory(jQuery.datepicker)}}(function(datepicker){datepicker.regional.ko={closeText:"닫기",prevText:"이전달",nextText:"다음달",currentText:"오늘",monthNames:["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"],monthNamesShort:["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"],dayNames:["일요일","월요일","화요일","수요일","목요일","금요일","토요일"],dayNamesShort:["일","월","화","수","목","금","토"],dayNamesMin:["일","월","화","수","목","금","토"],weekHeader:"주",dateFormat:"yy. m. d.",firstDay:0,isRTL:false,showMonthAfterYear:true,yearSuffix:"년"};datepicker.setDefaults(datepicker.regional.ko);return datepicker.regional.ko}));

/* jQuery Plugin : event.drag */
...
/* jQuery Plugin : clipboard.js v2.0.11 */
...
/* jQuery Plugin : UI Touch Punch 0.2.3 */
...
$(function(){$.fn.pcy_select=function(options,callback){return this.each(function(){var $el=$(this);$.fn.pcy_select.defaults={'theme':'pcy-select-gray','width':150,'height':30,'mode':''};var option=$.extend({},$.fn.pcy_select.defaults,options);var $slt=$('<div class="'+option.theme+'" style="cursor:pointer;display:inline-block;*display:inline;*zoom:1;width:'+option.width+'px;line-height:'+option.height+'px;position:relative;box-sizing:border-box;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;-o-box-sizing:border-box;-ms-box-sizing:border-box;"></div>');var slt_name=$el.attr('name');var slt_id=$el.attr('id');var $opts=$('option',$el);var $input=$('<input type="hidden" name="'+slt_name+'" id="'+slt_id+'" />');function init(){create_select();create_options();create_hidden_input();$opts_ul=$('> ul',$slt);$opts_ul=$('> ul',$slt);$opts_li=$('> ul > li',$slt);auto_selected();start()}function create_select(){$slt.insertAfter($el).html('<span>'+option.placeholder+'</span>')}function create_options(){var opt_count=$opts.length;var $opt_sum='<ul style="cursor:pointer;display:none;position:absolute;top:'+option.height+'px;left:0;width:'+option.width+'px;box-sizing:border-box;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;-o-box-sizing:border-box;-ms-box-sizing:border-box;">';for(var i=0;i<opt_count;i++){var opt_txt=$opts.eq(i).text();$opt_sum+='<li>'+opt_txt+'</li>'}$opt_sum+='</ul>';$slt.append($opt_sum)}function create_hidden_input(){$input.insertAfter($slt)}function click_select(){var visible=$opts_ul.is(':visible');if(visible){$opts_ul.hide()}else{switch(option.mode){case'slide':$opts_ul.slideDown(200);break;case'fade':$opts_ul.fadeIn(200);break;default:$opts_ul.show()}}}function click_option(idx){var opt_val=$opts.eq(idx).val();var opt_txt=$opts.eq(idx).text();$input.val(opt_val);$('> span',$slt).text(opt_txt)}function auto_selected(){var slt_std=$('> option:selected',$el).index();click_option(slt_std);$el.remove()}function start(){$slt.on('click',function(e){click_select()});$opts_li.click(function(e){var idx=$(this).index();click_option(idx)});$('body').click(function(e){if($slt.has(e.target).length==0&&!$slt.is(e.target)&&$opts_ul.has(e.target).length==0&&!$opts_ul.is(e.target)){$opts_ul.hide()}})}init()})}});
